...
export const getProductsDiscount = async (search) => {
    let res = {};
    if (search?.length > 0) {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/product/getProductDiscount?filter=name&filter=${search}`);
    } else {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/product/getProductDiscount`);
    }
    return res.data;
};
